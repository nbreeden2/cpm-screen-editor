...
#### 7. About (B)
- Displays `SEDIT v1.27 CP/M Screen Editor` on the status bar
...
Braces (`{` and `}`) are colored by nesting depth, cycling through red, green, and blue (depth mod 3). A closing brace matches the color of its corresponding opening brace. Brace nesting depth is tracked across lines via a pre-scan from the start of the file.
...
6. Close file; restore original user area
7. Move cursor to top; clear MODIFIED flag
8. If virtual mode was activated during loading, rebalance the buffer around line 1 via VIGOTO to provide ~4 KB of editing headroom
9. Call SYNINIT to detect syntax mode from extension
...
6. Write buffer contents record-by-record via GBLOGRD:
...
On CP/M, each variant has its own .SUB build script (runs SEPATCH + M80 + L80):

```
SUBMIT SEDIT        Mono (SEDIT.COM)
SUBMIT SEDITCL      Color (SEDIT-CL.COM)
SUBMIT SEDITA       ASM highlighting (SEDIT-A.COM)
SUBMIT SEDITC       C highlighting (SEDIT-C.COM)
SUBMIT SEADM31      ADM-31 (SEADM31.COM)
SUBMIT SEC3102      Cromemco 3102 (SEC3102.COM)
SUBMIT SEVT52       VT52 (SEVT52.COM)
SUBMIT SEPATCH      SEPATCH utility only
```
...
8. Detect terminal size via VT100 DSR: query row count and column width; set 132-column mode if detected; compute dynamic edit area rows; save initial column mode for restore on exit
