...
#### 7. About (B)
- Displays `SEDIT v1.19 CP/M Screen Editor` on the status bar
...
7. If virtual mode was activated during loading, rebalance the buffer around line 1 via VIGOTO to provide ~4 KB of editing headroom
...
7. Detect terminal size via VT100 DSR: query row count and column width; set 132-column mode if detected; compute dynamic edit area rows; save initial column mode for restore on exit
